...
const ProgressBar = require('../../lib/ProgressBar/ProgressBar.js');
...
DBMana.getDBList = async function () {
    let filePathList = await FSTool.getFileList(path.join(configTool.appBasePath, 'db'));
...
DBMana.createDB = async function (DBName, targetDirectory) {
    ...
    // 资源库 信息
    let DBFilePath = path.join(configTool.appBasePath, `\\db\\${DBName}.sqlite3`);
    let DBFileInfo = await FSTool.getPathInfo(DBFilePath, false);
    let res = false;
    ...
    if (targetDirectoryInfo && targetDirectoryInfo.isDirectory && DBFileInfo === null) {
        ...
        await dbToll.init();
        let pg = new ProgressBar('创建资源库', 25);
...
DBMana.exportDB = async function (DBName, targetDirectory) {
    ...
    if (targetDirectoryInfo && targetDirectoryInfo.isDirectory) {
        let dbToll = new DBTool(DBName);
        await dbToll.init();
        let pg = new ProgressBar('导出资源库', 25);
